...
async function doLogin(){
  ...
  if(!email||!pass){showAuthErr('login',t('fill_fields'));return;}

  // Show loading state
  const btn=document.querySelector('#form-login .btn-primary');
  const origText=btn.textContent; btn.textContent=t('logging_in'); btn.disabled=true;

  try {
    const userData = await loadUserFromDB(email);

    if (!userData) { showAuthErr('login', t('account_not_found')); return; }
    if (userData.pass !== btoa(pass)) { showAuthErr('login', t('wrong_password')); return; }
    ...
    if (!users[email]) { showAuthErr('login', t('account_not_found')); return; }
    if (users[email].pass !== btoa(pass)) { showAuthErr('login', t('wrong_password')); return; }
...
async function doRegister(){
  ...
  if(!name){showAuthErr('reg',t('fill_fields'));return;}
  ...
  const origText=btn.textContent; btn.textContent=t('creating'); btn.disabled=true;

  try {
    // Check if email already exists in Supabase
    const {data:existing} = await _SB.from('users').select('email').eq('email',email).maybeSingle();
    if(existing){showAuthErr('reg',t('email_used'));return;}
  } catch(e) {
    // Offline — check localStorage
    const users=loadUsers();
    if(users[email]){showAuthErr('reg',t('email_used'));return;}
  }
