...
async function sendFriendRequest(receiverEmail) {
  if (!U || U.isGuest) { toast('❌ ' + t('tab_login')); return; }
  if (receiverEmail === U.email) { toast('❌'); return; }
  ...
  try {
    ...
    if (existing) {
      if (existing.status === 'accepted') { toast('✅'); return; }
      if (existing.status === 'pending')  { toast('⏳'); return; }
    }
