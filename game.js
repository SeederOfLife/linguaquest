...
// ── Speech helpers ───────────────────────────────────────────────
// Reverse-lookup a word ID from its translated text + language
function _findWid(text,lang){
  if(!text||!lang) return null;
  const lower=text.toLowerCase();
  return Object.keys(WD).find(id=>WD[id][lang]&&WD[id][lang].toLowerCase()===lower)||null;
}
...
function mkQQ(id,all){
  const q=wt(id,S.nL),cor=wt(id,S.tL);
  const wrongIds=shuf(all.filter(x=>x!==id)).slice(0,3);
  const wr=wrongIds.map(x=>wt(x,S.tL));
  const allChoices=shuf([{wid:id,text:cor},...wrongIds.map((wid,i)=>({wid,text:wr[i]}))]);
  return{type:'quiz',wid:id,q,correct:cor,choices:allChoices.map(c=>c.text),choiceWids:allChoices.map(c=>c.wid)};
}
function mkFQ(id){return{type:'fill',wid:id,q:wt(id,S.nL),correct:wt(id,S.tL)};}
...
function renderQuiz(q){
  sT('g-text',q.q);
  // Add speaker icon next to question text
  renderSpeakIcon('g-text', q.q, S.nL);
  ...
  ['A','B','C','D'].forEach((l,i)=>{
    ...
    // Attach word ID and language for tooltip+TTS
    if(q.choiceWids&&q.choiceWids[i]){btn.dataset.wid=q.choiceWids[i];btn.dataset.lang=S.tL;}
    btn.onclick=(e)=>{
      onWordClick(btn,e);
      pickQ(q.choices[i],btn,q);
    };
...
function renderFill(q){
  sT('g-text',q.q);
  renderSpeakIcon('g-text', q.q, S.nL);
  const inp=$('fill-input');inp.value='';inp.className='fill-input';inp.disabled=false;
  setTimeout(()=>inp.focus(),70);showBtn('btn-check');startTimer(20);
}
...
function renderSort(q){sT('g-text',q.q);S.sortArr=[];const bank=$('word-bank');bank.innerHTML='';shuf([...q.words]).forEach(w=>{
    const t=document.createElement('div');t.className='word-token';t.textContent=w;
    // Try reverse-lookup word ID in WD
    const wid=_findWid(w,S.tL);if(wid){t.dataset.wid=wid;t.dataset.lang=S.tL;}
    t.onclick=(e)=>{
      if(typeof onWordClick!=='undefined') onWordClick(t,e);
      else if(typeof speakWord!=='undefined') speakWord(w,S.tL);
      addSort(w,t);
    };
    bank.appendChild(t);
  });$('sentence-slots').innerHTML='<span class="slot-placeholder">Clique sur les mots…</span>';showBtn('btn-check');showBtn('btn-reset');startTimer(30);}
...
function mkMI(it,side){
  const d=document.createElement('div');d.className='match-item';d.textContent=it.text;
  d.dataset.id=it.id;d.dataset.side=side;
  d.dataset.wid=it.id;d.dataset.lang=side==='left'?S.nL:S.tL;
  d.onclick=(e)=>{
    if(typeof onWordClick!=='undefined') onWordClick(d,e);
    clickMatch(d);
  };
  return d;
}
...
function doFill(q){clearInterval(S.timer);const inp=$('fill-input'),res=fuzzy(inp.value,q.correct);inp.disabled=true;if(res==='exact'){inp.className='fill-input correct';S.score+=10;S.cor++;S.ts.fill.c++;showFB(true,'🎉 Parfait !',q.correct);}
  else if(res==='close'){inp.className='fill-input close';S.score+=5;S.cor++;S.ts.fill.c++;const fn=$('fuzzy-note');fn.textContent=`✨ Presque ! Réponse exacte : ${q.correct}`;fn.style.display='block';showFB('close','✨ Presque !',`Réponse exacte : ${q.correct}`);}
  else{inp.className='fill-input wrong';S.wr++;S.ts.fill.w++;showFB(false,'❌ Pas tout à fait !','Réponse : '+q.correct);}
  // Speak correct answer in target language
  if(typeof speakAnswer!=='undefined') speakAnswer(q.correct,S.tL);
  // Show tooltip for this word near input
  if(q.wid&&typeof showWordTooltip!=='undefined') setTimeout(()=>showWordTooltip(inp,q.wid,S.tL,S.nL),300);sT('g-score',S.score);hideBtn('btn-check');showBtn('btn-next');}
...
function showResults(){
  ...
  if(typeof speakResult!=='undefined') speakResult(stars>=2, S.nL);
}
